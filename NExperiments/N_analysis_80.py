from utils import RL_with_constraint, RL_without_constraint, RLWKC_with_constraint, RLWKC_without_constraint, HTP, sparsePermutation, addNoise, meanNormalizedError, sgn, one_norm, only_priors, HTP_with_priors
import numpy as np
...
L2HTPErrorsWithoutPriorInfo = np.zeros(shape = (permutation_runs, noise_runs))
...
for N in [124]:
    ...
    for per_run in range(permutation_runs):
        ...
        for noise_run in range(noise_runs):
            ...
            xhatL2HTPWithoutPriorInfo, _ = HTP_with_priors(A, noisy_y, A_cv, y_cv, 0)
            L2HTPErrorsWithoutPriorInfo[per_run][noise_run] = meanNormalizedError(x, xhatL2HTPWithoutPriorInfo)
            print(L2HTPErrorsWithoutPriorInfo[per_run][noise_run])
            ...
            np.save('L2HTPErrorsWithoutPriorInfo' + '.npy', L2HTPErrorsWithoutPriorInfo)
