...
def sparsePermutation(N, m, noPermuted):
    ...
    '''
    to_be_permuted = random.sample(range(m, 450), noPermuted)
    P = np.eye(N)
...
def BayesianLearning(phi, y, sigma):
    ...
    while (iteration <= 1) or ((np.linalg.norm(mu - mu_prev)/np.linalg.norm(mu)) >= 0.01):
        mu_prev = np.copy(mu)
        ...
        # if iteration >= 3:
        #     print(np.linalg.norm(mu - mu_prev), np.linalg.norm(mu - mu_prev)/np.linalg.norm(mu))
